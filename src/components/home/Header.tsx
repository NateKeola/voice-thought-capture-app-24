  ...
  return <>
      <div className="bg-white px-6 pt-10 pb-4 shadow-sm flex flex-col items-center">
        <div className="w-full flex justify-center items-center relative mb-1">
          ...
          <h1 className="text-gray-900 text-3xl font-extrabold tracking-wider text-center w-full" style={{
          letterSpacing: '0.1em'
        }}>memo</h1>
          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-2">
            <button onClick={() => setShowMemoAI(true)} className="bg-orange-100 w-10 h-10 rounded-full flex items-center justify-center hover:bg-orange-200 transition-colors" title="Chat with Memo AI">
              <Brain className="h-5 w-5 text-orange-600" />
            </button>
            <button onClick={handleProfileClick} className="bg-orange-100 w-10 h-10 rounded-full flex items-center justify-center">
      ...
      {showMemoAI && <MemoAIChat onClose={() => setShowMemoAI(false)} />}
    </>;
};
